...
const required = ["ISIN", "NSE_Symbol", "v3_Final_Score", "v3_Final_Rating"];
for (const col of required) {
  if (C[col] === undefined) {
    console.error(`Column "${col}" not found in CSV. Available: ${headers.join(", ")}`);
    process.exit(1);
  }
}
...
for (let i = 1; i < lines.length; i++) {
  ...
  const nseSymbol = (row[C["NSE_Symbol"]] || "").trim();
  const bseCode = (row[C["BSE_Code"]] || "").trim();
  const stockName = (row[C["Stock_Name"]] || "").trim();

  const v3Score = toNum(row[C["v3_Final_Score"]]);
  const v3Rating = (row[C["v3_Final_Rating"]] || "").trim();
  ...
  const entry = {
    ...
    fundamentals: {
      ...
      pb: toNum(row[C["PB"]]),
      evEbitda: toNum(row[C["EV_EBITDA"]]),
